...
import com.beldex.libsignal.utilities.Base64.encodeBytes
import com.beldex.libsignal.utilities.HTTP
import com.beldex.libsignal.utilities.HTTP.Verb.DELETE
import com.beldex.libsignal.utilities.HTTP.Verb.GET
import com.beldex.libsignal.utilities.HTTP.Verb.POST
import com.beldex.libsignal.utilities.HTTP.Verb.PUT
...
object OpenGroupAPIV2 {
    ...
    private fun getResponseBody(request: Request): Promise<ByteArray, Exception> {
        return send(request).map { json ->
            val result = json.info["result"] as? String ?: throw Error.ParsingFailed
            decode(result)
        }
       /* return send(request).map { response ->
