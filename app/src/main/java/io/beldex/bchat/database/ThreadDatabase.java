...
import static io.beldex.bchat.database.GroupDatabase.GROUP_ID;
...
public class ThreadDatabase extends Database {
  ...
  public List<MessagingDatabase.MarkedMessageInfo> setRead(long threadId, boolean lastSeen) {
    ...
    final List<MessagingDatabase.MarkedMessageInfo> smsRecords = DatabaseComponent.get(context).smsDatabase().setMessagesRead(threadId);
    final List<MessagingDatabase.MarkedMessageInfo> mmsRecords = DatabaseComponent.get(context).mmsDatabase().setMessagesRead(threadId);
  ...
  public void deleteConversation(long threadId) {
    ...
    DatabaseComponent.get(context).beldexMessageDatabase().deleteThread(threadId);
    deleteThread(threadId);
    notifyConversationListeners(threadId);
    notifyConversationListListeners();
    BchatMetaProtocol.clearReceivedMessages();
  }
  ...
  public void markAllAsRead(long threadId, boolean isGroupRecipient) {
    List<MessagingDatabase.MarkedMessageInfo> messages = setRead(threadId, true);
    if (isGroupRecipient) {
      for (MessagingDatabase.MarkedMessageInfo message: messages) {
  ...
  public class Reader implements Closeable {
    ...
    public ThreadRecord getCurrent() {
      ...
      if (count > 0) {
        MmsSmsDatabase mmsSmsDatabase = DatabaseComponent.get(context).mmsSmsDatabase();
        long messageTimestamp = mmsSmsDatabase.getLastMessageTimestamp(threadId, false, false);
