...
import com.thoughtcrimes.securesms.animation.AnimationCompleteListener
import com.thoughtcrimes.securesms.animation.ResizeAnimation
...
class KeyboardPageSearchView @JvmOverloads constructor(
  ...
  fun enableBackNavigation(enable: Boolean = true) {
    navButton.setImageResource(if (enable) R.drawable.ic_arrow_left_24 else R.drawable.ic_baseline_search_24)
    if (enable) {
      navButton.setImageResource(R.drawable.ic_arrow_left_24)
      navButton.setOnClickListener { callbacks?.onNavigationClicked() }
    } else {
      navButton.setImageResource(R.drawable.ic_baseline_search_24)
