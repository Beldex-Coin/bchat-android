...
import com.thoughtcrimes.securesms.search.model.MessageResult;
...
import com.thoughtcrimes.securesms.database.CursorList;
...
public class SearchRepository {
  ...
  private final BchatContactDatabase contactDatabase;
  ...
  public SearchRepository(@NonNull Context context,
                          ...
                          @NonNull BchatContactDatabase contactDatabase,
  ...
  private Pair<CursorList<Contact>, List<String>> queryContacts(String query) {
    ...
    while (contacts.moveToNext()) {
      try {
        ...
        String contactBchatId = contact.getBchatID();
        Address address = Address.fromSerialized(contactBchatId);
        contactList.add(address);
        contactStrings.add(contactBchatId);
      } catch (Exception e) {
        Log.e("Beldex", "Error building Contact from cursor in query", e);
      }
    }
  ...
  private static class ContactModelBuilder implements CursorList.ModelBuilder<Contact> {
    ...
    private final BchatContactDatabase contactDb;
    private final ThreadDatabase threadDb;

    public ContactModelBuilder(BchatContactDatabase contactDb, ThreadDatabase threadDb) {
    ...
    public Contact build(@NonNull Cursor cursor) {
      ...
      Contact contact = contactDb.getContactWithBchatID(threadRecord.getRecipient().getAddress().serialize());
      if (contact == null) {
