...
class ReactionDatabase(context: Context, helper: SQLCipherOpenHelper) : Database(context, helper) {
    ...
    fun addReaction(messageId: MessageId, reaction: ReactionRecord, notifyUnread: Boolean) {
        ...
        try {
            ...
            if (messageId.mms) {
                DatabaseComponent.get(context).mmsDatabase().updateReactionsUnread(databaseHelper.writableDatabase, messageId.id, hasReactions(messageId), false, notifyUnread)
            } else {
                DatabaseComponent.get(context).smsDatabase().updateReactionsUnread(databaseHelper.writableDatabase, messageId.id, hasReactions(messageId), false, notifyUnread)
            }
    ...
    private fun deleteReactions(messageId: MessageId, query: String, args: Array<String>, notifyUnread: Boolean) {
        ...
        try {
            ...
            if (messageId.mms) {
                DatabaseComponent.get(context).mmsDatabase().updateReactionsUnread( databaseHelper.writableDatabase, messageId.id, hasReactions(messageId), true,notifyUnread)
            } else {
                DatabaseComponent.get(context).smsDatabase().updateReactionsUnread( databaseHelper.writableDatabase, messageId.id, hasReactions(messageId), true,notifyUnread)
            }
