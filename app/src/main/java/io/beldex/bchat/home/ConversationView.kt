...
class ConversationView : LinearLayout {
    ...
    fun bind(thread: ThreadRecord, isTyping: Boolean, glide: RequestManager) {
        ...
        if (thread.isSharedContact || isSharedContact(thread.body)) {
            ...
            // Apply mentions (this modifies text)
            val snippet = highlightMentionsSpannableString(formatted,thread.threadId, context)
            // (optional) apply block quote again
            val isAttachment = attachmentRegex
                .containsMatchIn(snippet.toString())
            if (isAttachment) {
                TextFormatter.applyInlineBlockQuote(context, snippet, attachmentRegex)
            }
