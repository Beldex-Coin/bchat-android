...
import com.thoughtcrimes.securesms.imageeditor.renderers.TextRenderer;
...
public final class ImageEditorView extends FrameLayout {
  ...
  @Nullable
  private EditBchat editBchat;
  private boolean     moreThanOnePointerUsedInBchat;
  ...
  public boolean onTouchEvent(MotionEvent event) {
    switch (event.getActionMasked()) {
      case MotionEvent.ACTION_DOWN: {
        ...
        moreThanOnePointerUsedInBchat = false;
        model.pushUndoPoint();
        editBchat = startEdit(inverse, point, selected);

        if (tapListener != null && allowTaps()) {
          if (editBchat != null) {
            tapListener.onEntityDown(editBchat.getSelected());
          } else {
            tapListener.onEntityDown(null);
          }
        }
        ...
        if (editBchat != null) {
          ...
          for (int h = 0; h < historySize; h++) {
            for (int p = 0; p < pointerCount; p++) {
              editBchat.movePoint(p, getHistoricalPoint(event, p, h));
            }
          }

          for (int p = 0; p < pointerCount; p++) {
            editBchat.movePoint(p, getPoint(event, p));
          }
          model.moving(editBchat.getSelected());
          invalidate();
          return true;
        }
        break;
      ...
      case MotionEvent.ACTION_POINTER_DOWN: {
        if (editBchat != null && event.getPointerCount() == 2) {
          moreThanOnePointerUsedInBchat = true;
          editBchat.commit();
          model.pushUndoPoint();

          Matrix newInverse = model.findElementInverseMatrix(editBchat.getSelected(), viewMatrix);
          if (newInverse != null) {
            editBchat = editBchat.newPoint(newInverse, getPoint(event, event.getActionIndex()), event.getActionIndex());
          } else {
            editBchat = null;
          }
          if (editBchat == null) {
      ...
      case MotionEvent.ACTION_POINTER_UP: {
        if (editBchat != null && event.getActionIndex() < 2) {
          editBchat.commit();
          model.pushUndoPoint();
          dragDropRelease();
          ...
          Matrix newInverse = model.findElementInverseMatrix(editBchat.getSelected(), viewMatrix);
          if (newInverse != null) {
            editBchat = editBchat.removePoint(newInverse, event.getActionIndex());
          } else {
            editBchat = null;
          }
          return true;
      ...
      case MotionEvent.ACTION_UP: {
        if (editBchat != null) {
          ...
          editBchat = null;
          model.postEdit(moreThanOnePointerUsedInBchat);
          invalidate();
          return true;
        } else {
          model.postEdit(moreThanOnePointerUsedInBchat);
  ...
  private @Nullable
  EditBchat startEdit(@NonNull Matrix inverse, @NonNull PointF point, @Nullable EditorElement selected) {
    if (mode == Mode.Draw) {
      return startADrawingBchat(point);
    } else {
      return startAMoveAndResizeBchat(inverse, point, selected);
    }
  }

  private EditBchat startADrawingBchat(@NonNull PointF point) {
    ...
    return DrawingBchat.start(element, renderer, elementInverseMatrix, point);
  }

  private EditBchat startAMoveAndResizeBchat(@NonNull Matrix inverse, @NonNull PointF point, @Nullable EditorElement selected) {
    ...
    if (selected.getRenderer() instanceof ThumbRenderer) {
      ...
      if (elementInverseMatrix != null) {
        return ThumbDragEditBchat.startDrag(selected, elementInverseMatrix, thumb.getControlPoint(), point);
      } else {
        return null;
      }
    }

    return ElementDragEditBchat.startDrag(selected, inverse, point);
  }
  ...
  private final class DoubleTapGestureListener extends GestureDetector.SimpleOnGestureListener {
    ...
    public boolean onDoubleTap(MotionEvent e) {
      if (tapListener != null && editBchat != null && allowTaps()) {
        tapListener.onEntityDoubleTap(editBchat.getSelected());
      }
      return true;
    }
    ...
    public boolean onSingleTapUp(MotionEvent e) {
      if (tapListener != null && allowTaps()) {
        if (editBchat != null) {
          EditorElement selected = editBchat.getSelected();
