...
class InputBarButton : RelativeLayout {
    ...
    private var isMessageBox= false
    ...
    //New Line
    private val sendImageViewContainer by lazy {
        val result = InputBarButtonSendImageViewContainer(context)
        val size = collapsedSize.toInt()
        result.layoutParams = LayoutParams(size, size)
        if (!isSendButton) {
            result.setBackgroundResource(R.drawable.input_bar_button_background)
    ...
    private val imageView by lazy {
        ...
        //val colorID = if (isSendButton) R.color.text else R.color.text
